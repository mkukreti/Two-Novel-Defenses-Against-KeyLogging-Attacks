                        ...
                        if (sensorObject != null) {
                            ...
                            for (int i=0 ; i<size ; i++) {
                                ...
                                if (listener.hasSensor(sensorObject)) {
                                    /* y59song - modify start */
                                    // TODO : do some modifications with the values
                                    // Opt 1 : check the degree of modification
                                    Random random = new Random();
                                    float noiseRate = random.nextFloat()/10;
                                    switch(sensorObject.getType()) {
                                        case Sensor.TYPE_ACCELEROMETER:
                                            for(int j = 0; j<3; j++) values[j] = values[j] * (random.nextBoolean() ? 1+noiseRate : 1-noiseRate);
                                            break;
                                        case Sensor.TYPE_ORIENTATION:
                                            for(int j = 0; j<3; j++) values[j] = values[j] * (random.nextBoolean() ? 1+noiseRate/10 : 1-noiseRate/10);
                                            break;
                                        default:
                                            break;
                                    }
                                    // Opt 2 : Just mask all small change in accelerometer sensor event, then can not detect tap event
                                    /*
                                    if(sensorObject.getType() == Sensor.TYPE_ACCELEROMETER) {
                                        double sqsum = 0;
                                        for(int j = 0; j < 3; j ++) sqsum += values[j] * values[j];
                                        if(sqsum < 130 && sqsum > 90) {
                                            float ratio = (float)Math.sqrt(sqsum) / 9.8;
                                            for(int j = 0; j < 3; j ++) values[j] /= ratio;
                                        }
                                    }
                                    */
                                    /* y59song - modify end */
                ...
                public void handleMessage(Message msg) {
                    ...
                    switch (t.sensor.getType()) {
                        ...
                        case Sensor.TYPE_ACCELEROMETER:
                            /* y59song - modify start */
                            double sqsum = 0;
                            for(int j = 0; j < 3; j ++) sqsum += t.values[j] * t.values[j];
                            if(sqsum < 130 && sqsum > 80) {
                                float ratio = (float)(Math.sqrt(sqsum) / 9.8);
                                for(int j = 0; j < 3; j ++) t.values[j] /= ratio;
                            }
                            /* y59song - modify end */
                        default:
                    ...
                    /* y59song modify start */
                    synchronized(inputLock) {
                        Log.v(TAG, "input active before strange set : " + inputActive.value + " " + SensorManager.inputActive.value + " " + inputActive.hashCode() + " " + SensorManager.inputActive.hashCode());
                        //boolean temp = InputMethodManager.inputActive;
                        //Log.v(TAG, "input_method before strange set : " + temp);
                        if(!inputActive.value) mSensorEventListener.onSensorChanged(t);
                        //if(!inputActive.value && !(SensorManager.inputActive.value)) mSensorEventListener.onSensorChanged(t);
                    }
                    /* y59song modify end */
                    returnToPool(t);
                }
